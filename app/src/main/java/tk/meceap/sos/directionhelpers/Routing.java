package tk.meceap.sos.directionhelpers;
...
public class Routing extends AsyncTask<LatLng, Void, Route>
{
    ...
    protected Route doInBackground(final LatLng... points) {
        try {
            ...
            sBuf.append("&sensor=true&mode=driving&key="+R.string.google_maps_key);
            System.out.println("sbuf: " + sBuf);
            parser = new GoogleParser(sBuf.toString());
            final Route route = (Route) parser.parse();
            return route;
    ...
    protected void onPostExecute(final Route result) {
        ...
        try {
            if (result == null) {
                ...
                final String text = String.valueOf(result.getLength());
                final String startAddress = "You location";
                final String endAddress = result.getEndAddressText();
                txtDistance.setVisibility(View.GONE);
                if (text != null) {
                    txtDistance.setVisibility(View.VISIBLE);
                    txtDistance.setBackgroundResource(android.R.color.holo_orange_light);
                    txtDistance.setText(" Total Distance : " + text + "\n Total Duration : " + result.getDurationText() + "  ");
                }
                ...
                final Marker startLocation = map.addMarker(new MarkerOptions().position(start).title(startAddress).snippet("Main Location").icon(BitmapDescriptorFactory.fromResource(R.drawable.start_location)));
                final Marker endLocation = map.addMarker(new MarkerOptions().position(dest).title(endAddress).snippet("Destination Location").icon(BitmapDescriptorFactory.fromResource(R.drawable.end_location)));
